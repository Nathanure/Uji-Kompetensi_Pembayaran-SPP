...
function morganConfig (tokens, req, res) {
    ...
    if(req.session.admin) {
        ...
        const role = req.session.admin.tingkatan;
        // If 304
        if (length === undefined) {
    ...
    // With admin session
        ...
        const name = req.session.petugas.username
        const role = req.session.petugas.tingkatan
        // If 304
        if (length === undefined) {
            // logdb(name, role, date, method, url, status, 0, rstime, origin, address)
            return [chalk.whiteBright(name), chalk.magenta(role), chalk.hex('#f78fb3').bold(date), chalk.hex('#34ace0').bold(method), chalk.hex('#ff5252').bold(url), chalk.hex('#ffb142').bold(status), chalk.cyanBright('0'), chalk.hex('#2ed573').bold(rstime), chalk.hex('#fffa65').bold(origin), chalk.yellow(address)].join(' ');
    ...
    // With user session
        ...
        const name = req.session.siswa.id_nis;
        // If 304
        if (length === undefined) {
            // logdb(name, role, date, method, url, status, 0, rstime, origin, address)
            return [chalk.whiteBright(name), chalk.magenta('siswa'), chalk.hex('#f78fb3').bold(date), chalk.hex('#34ace0').bold(method), chalk.hex('#ff5252').bold(url), chalk.hex('#ffb142').bold(status), chalk.cyanBright('0'), chalk.hex('#2ed573').bold(rstime), chalk.hex('#fffa65').bold(origin), chalk.yellow(address)].join(' ');
        // If !304
        } else {
            // logdb(name, role, date, method, url, status, length, rstime, origin, address)
            return [chalk.whiteBright(name), chalk.magenta('siswa'), chalk.hex('#f78fb3').bold(date), chalk.hex('#34ace0').bold(method), chalk.hex('#ff5252').bold(url), chalk.hex('#ffb142').bold(status), chalk.cyanBright(length), chalk.hex('#2ed573').bold(rstime), chalk.hex('#fffa65').bold(origin), chalk.yellow(address)].join(' ');
        }    
    // With no session
    } else {
        if (length === undefined) {
